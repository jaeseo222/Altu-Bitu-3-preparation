...
* 1. 맵의 key에 문자를, value에 문자의 인덱스들을 저장
* 2. 윈도우 크기를 K로 설정해서, 각 문자마다 슬라이딩 윈도우를 통해 문자열 길이 계산
* 어떤 문자 K개를 포함하는 가장 짧은 연속 문자열의 길이 -> min 함수로 계산
* 어떤 문자 K개를 포함하고 처음과 끝이 해당 문자인 가장 긴 연속 문자열의 길이 -> max 함수로 계산
*/
