...
/**
 * 주변인들이 얼마나 믿는지 배열을 통해 관리.
 * 방문 체크 배열을 루머를 믿는 시간을 저장하는 배열로 대체
 *    ans[i] = -1 일 경우, i는 루머를 믿지 않음
 *    ans[i] = t인 경우, i는 t분 부터 루머를 믿기 시작함
 * 각자가 루머를 믿기 위해 주변인의 절반 이상이 루머를 믿어야 하므로, 각 사람이 루머를 믿기까지 루머를 믿는 주변인 몇명이 남았는지를 리스트에 기록
